...
export default function HomePage() {
  ...
  function continueToNextLevel() { const next=level+1;if(next===2){setCurrentLevel(2);setTasks(level2Tasks);setEmailCursor(5)}else if(next===3){setCurrentLevel(3);setTasks(level3Tasks);setEmailCursor(8)}else{setFinalReady(true)}setCelebrate(null);setClaimedReward("");setPreviewReward("");setShowMerch(false);if(next<=3){setToast(`Level ${next} · ${next===2?"Academic Takeoff":"Welcome Home"} unlocked!`);setTimeout(()=>setToast(""),3000);setTimeout(()=>setEmailPopup(sampleEmails[next===2?5:8]),1600)}}
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {showMerch ? <section className="merch-panel"><div className="merch-panel-head"><div><div className="eyebrow"><span/> YOUR GT MERCH</div><h2>{studentName || "Your"}’s reward closet</h2><p>Complete levels, claim coupons, and build your Georgia Tech collection.</p></div><div className="merch-count"><small>COUPONS CLAIMED</small><b>{merchItems.length} / 3</b><span>One free demo item per level</span></div></div><div className="merch-showcase">{merchItems.length?merchItems.map((item,i)=><motion.article key={item} initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:i*.08}}><div>{merchEmoji[item]}</div><small>LEVEL {i+1} REWARD</small><h3>{item}</h3><span>✓ COUPON CLAIMED</span></motion.article>):<div className="empty-merch"><span>🎟️</span><h3>Your merch closet is ready</h3><p>Complete Level 1 to unlock your first free GT merch coupon.</p><button onClick={()=>setShowMerch(false)}>Return to journey <ArrowRight/></button></div>}</div><div className="merch-disclaimer"><Sparkles/><p><b>Hackathon reward concept</b>Coupons and merchandise shown in Buzz Map are simulated and are not official Georgia Tech offers.</p></div></section> : <section className="journey-panel">
