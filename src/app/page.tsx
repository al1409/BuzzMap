...
const level3Tasks: Task[] = [
  ...
  {id:"l3-3",title:"Coordinate Shared Dorm Items",description:"Decide who will bring shared items such as a rug, fan, cleaning supplies, or small appliances.",category:"Housing",deadline:"2026-08-07",points:35,completed:false,order:2},
  ...
  {id:"l3-7",title:"Move In and Make It Home",description:"Check in, meet your community, unpack, and add the finishing touches to your dream dorm.",category:"Housing",deadline:"2026-08-15",points:100,completed:false,order:6},
];
...
const roomRewards: Record<number, string[]> = {2:["Desk Lamp","Snake Plant","Buzz Plush"],3:["GT Poster","Football Poster","Gaming Poster"],4:["Bean Bag","Reading Chair","Gaming Chair"],5:["Bookshelf","Dual Monitors","TV"],6:["Mini Fridge","Coffee Station","Snack Shelf"],7:["LED Lights","Curtains","Wall Shelves"],8:["Mechanical Keyboard","Whiteboard","Storage Cubes"],9:["Gold Rug","Laundry Basket","Mirror"],10:["Ramblin’ Wreck Model","Premium GT Neon","Buzz Companion"]};
const rewardEmoji: Record<string,string> = {"Desk Lamp":"💡","Snake Plant":"🪴","Buzz Plush":"🐝","GT Poster":"GT","Football Poster":"🏈","Gaming Poster":"🎮","Bean Bag":"🟡","Reading Chair":"🪑","Gaming Chair":"💺","Bookshelf":"📚","Dual Monitors":"🖥️","TV":"📺","Mini Fridge":"🧊","Coffee Station":"☕","Snack Shelf":"🍿","LED Lights":"✨","Curtains":"▥","Wall Shelves":"▰","Mechanical Keyboard":"⌨️","Whiteboard":"▱","Storage Cubes":"▦","Gold Rug":"▭","Laundry Basket":"🧺","Mirror":"🪞","Ramblin’ Wreck Model":"🚘","Premium GT Neon":"⚡","Buzz Companion":"🐝"};
...
function DormRoom({items, level, preview}: {items:string[];level:number;preview?:string}) {
  const all = preview ? [...items,preview] : items;
  return <div className={`dorm-room stage-${Math.min(3,Math.floor(level/3))}`}><div className="room-wall"><div className="room-window"><i/><span>{level>=7?"CAMPUS VIEW":"ATLANTA"}</span></div><div className="room-bed"><i/><b/></div><div className="room-desk"><i/><b/></div><div className="room-chair"/><div className="room-items">{all.map((item,i)=><motion.div layout initial={{scale:0,y:-12}} animate={{scale:1,y:0}} className={`room-item item-${i%8}`} key={item} title={item}><span>{rewardEmoji[item]||"✦"}</span><small>{item}</small></motion.div>)}</div>{level>=7&&<div className="room-led"/>}</div><div className="room-floor"/></div>
}
...
export default function HomePage() {
  ...
  const [roomItems, setRoomItems] = useState<string[]>(() => savedState()?.roomItems || []);
  ...
  const levelRewardClaimed = (roomRewards[level+1] || []).some(item=>roomItems.includes(item));
  ...
  useEffect(() => { localStorage.setItem("first-flight-state", JSON.stringify({version:2,currentLevel,emailCursor,tasks,sideMissions,avatarColor,accessory,onboarded,guideDismissed,avatarType,skin,hair,roomItems,studentName,major})); }, [currentLevel,emailCursor,tasks,sideMissions,avatarColor,accessory,onboarded,guideDismissed,skin,hair,roomItems,studentName,major]);
  useEffect(() => {
    if (!onboarded || !guideDismissed || emailCursor !== emailStart || emailPopup) return;
    const timer = window.setTimeout(() => setEmailPopup(sampleEmails[emailStart]), 1400);
    return () => window.clearTimeout(timer);
  }, [onboarded, guideDismissed, emailCursor, emailPopup, emailStart]);

  function chooseReward(item:string) { setRoomItems(current=>[...current.filter(x=>!(roomRewards[celebrate||0]||[]).includes(x)),item]); setPreviewReward(item); setClaimedReward(item); setToast(`${item} added to your dorm · Level 2 is ready!`); setTimeout(()=>setToast(""),2800); }
  function continueToNextLevel() { const next=level+1;if(next===2){setCurrentLevel(2);setTasks(level2Tasks);setEmailCursor(5)}else if(next===3){setCurrentLevel(3);setTasks(level3Tasks);setEmailCursor(8)}setCelebrate(null);setClaimedReward("");setPreviewReward("");setShowRoom(false);setToast(`Level ${next} · ${next===2?"Academic Takeoff":"Welcome Home"} unlocked!`);setTimeout(()=>setToast(""),3000);setTimeout(()=>setEmailPopup(sampleEmails[next===2?5:8]),1600);}
  function resumeCompletedLevel() { if(level===3){setFinalReady(true);return}if(levelRewardClaimed) continueToNextLevel(); else { setClaimedReward(""); setPreviewReward(""); setCelebrate(level+1); } }
  ...
  function completeTask(task: Task) {
    ...
    if (!task.completed) {
      ...
      if (updated.every(t=>t.completed)) setTimeout(() => level===3?setFinalReady(true):setCelebrate(level + 1), 1750);
    }
  }
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      <div className="stat-row">
        ...
        <div className="next-reward-card"><span>{level===1?"🪴":level===2?"🖼️":"🐝"}</span><div><small>{level===3?"FINAL MILESTONE":"NEXT REWARD"}</small><b>{level===1?"Dorm Furniture Set":level===2?"Poster Collection":"Freshman Ready"}</b><em>{level===1?"+ GT T-shirt":level===2?"+ GT Hoodie":"Welcome Home badge"}</em></div></div>
      </div>
      ...
      {showRoom ? <section className="dorm-panel"><div className="dorm-panel-head"><div><div className="eyebrow"><span/> YOUR DREAM DORM</div><h2>{studentName || "Your"}’s room</h2><p>Every mission brings your space closer to home.</p></div><div className="next-unlock"><small>NEXT ROOM DROP</small><b>Level {Math.min(10,level+1)}</b><span>{Math.max(1,6-(completed%6))} missions away</span></div></div><DormRoom items={roomItems} level={level}/><div className="room-collection"><b>Your collection</b><span>{roomItems.length} items placed · Room evolves at levels 3, 6, and 9</span><div>{roomItems.length?roomItems.map(item=><i key={item}>{rewardEmoji[item]} <small>{item}</small></i>):<p>Complete your next mission to start making this space yours.</p>}</div></div></section> : <section className="journey-panel">
        ...
        <div className="map-wrap">
          ...
          <div className="map-mission-count"><Check/><span><b>{completed} / {tasks.length}</b> MISSIONS COMPLETE</span></div><button className="add-floating" onClick={()=>setShowAdd(true)}><Plus/> Add a mission</button>{emailCursor<emailEnd&&<button className="simulate-email" onClick={()=>setEmailPopup(sampleEmails[emailCursor])}><span>✦</span> Simulate Level {level} email <small>{emailCursor-emailStart+1}/{emailEnd-emailStart}</small></button>}
        </div>
      </section>}
      {!showRoom && sideMissions.length>0 && <section className="side-missions"><div className="side-heading"><div><Sparkles/><span><small>EMAIL-DISCOVERED OPPORTUNITIES</small><h2>Side Missions</h2></span></div><b>{sideMissions.filter(t=>t.completed).length}/{sideMissions.length}</b></div><div className="side-grid">{sideMissions.map(task=><button key={task.id} onClick={()=>setSelected(task)} className={task.completed?"complete":""}><span>{categoryMeta[task.category].icon}</span><div><small>OPTIONAL · +{task.points} XP</small><b>{task.title}</b><em>{new Date(`${task.deadline}T12:00:00`).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</em></div><ChevronRight/></button>)}</div><p>Side missions award bonus XP but never block level progress.</p></section>}
    </section>

    <AnimatePresence>
      {emailPopup && <><motion.div className="email-scrim" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/><motion.div className="email-detected" initial={{opacity:0,scale:.9,y:35}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.94,y:20}} transition={{type:"spring",damping:24}}><div className="email-radar"><i/><span>✦</span></div><small>SIMULATED INBOX ASSISTANT</small><h2>{emailPopup.kind==="side"?"New side mission available!":"Mission update detected"}</h2><div className="email-envelope"><div><span>FROM</span><b>{emailPopup.sender}</b></div><div><span>SUBJECT</span><b>{emailPopup.subject}</b></div><p>{emailPopup.preview}</p></div><div className="extracted-task"><span>AI EXTRACTED</span><h3>{emailPopup.title}</h3><p>{emailPopup.action}</p><div><b>📅 {new Date(`${emailPopup.deadline}T12:00:00`).toLocaleDateString("en-US",{month:"long",day:"numeric"})}</b><b>✨ +{emailPopup.points} XP</b></div></div><div className="email-note">{emailPopup.kind==="enhance"?"This matches a required mission, so First Flight will update it instead of creating a duplicate.":"This is optional and will never count against Level 1 progress."}</div><div className="email-actions"><button onClick={()=>resolveEmail(false)}>Dismiss</button><button onClick={()=>resolveEmail(true)}>{emailPopup.kind==="enhance"?"Update mission":"Accept mission"}<ArrowRight/></button></div></motion.div></>}
      {onboarded && !guideDismissed && <motion.div className="progress-guide" initial={{opacity:0}} animate={{opacity:1}}><motion.div initial={{scale:.92,y:22}} animate={{scale:1,y:0}}><div className="guide-badge">01</div><small>YOUR FIRST FLIGHT</small><h2>Welcome aboard, {studentName}!</h2><p>Your journey to becoming a Yellow Jacket is divided into levels. Each level contains important onboarding missions to help you arrive prepared.</p><div className="guide-loop"><div><span>✓</span><b>Complete missions</b><small>Finish essential GT onboarding tasks</small></div><ArrowRight/><div><span>XP</span><b>Earn experience</b><small>Watch your journey come to life</small></div><ArrowRight/><div><span>★</span><b>Unlock rewards</b><small>Personalize your avatar and dorm</small></div></div><div className="guide-rewards"><div><span>👕</span><p><b>Avatar cosmetics</b>Unlock Georgia Tech gear as you advance.</p></div><div><span>🛏️</span><p><b>Your dream dorm</b>Choose new furniture every level. By Move-In Day, it’ll be completely yours.</p></div></div><button className="complete-button" onClick={()=>setGuideDismissed(true)}>Begin Level 1 <ArrowRight/></button></motion.div></motion.div>}
      ...
      {selected && <><motion.div className="scrim" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setSelected(null)}/><motion.aside className="task-drawer" initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:260}}>
        ...
        {selected.emailSource&&<div className="email-context"><span>✦ UPDATED FROM GT EMAIL</span><b>{selected.emailSubject}</b><small>{selected.emailSource}</small><p><strong>Action required:</strong> {selected.actionRequired}</p><button onClick={()=>{setToast("Demo email preview opened");setTimeout(()=>setToast(""),2200)}}>View email summary <ArrowRight/></button></div>}
      ...
      {celebrate && <motion.div className="celebration reward-celebration" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>{Array.from({length:24}).map((_,i)=><motion.i key={i} style={{left:`${10+(i*37)%80}%`,background:["#edbd66","#fff0b9","#65c1b4","#d887aa"][i%4]}} initial={{y:-80,rotate:0}} animate={{y:"105vh",rotate:720}} transition={{duration:2+(i%5)*.31,delay:(i%7)*.07}}/>)}<motion.div className="reward-card" initial={{scale:.5,y:30}} animate={{scale:1,y:0}} transition={{type:"spring",delay:.15}}><Sparkles/><small>🎉 CONGRATULATIONS!</small><h2>You completed Level {celebrate-1}: {celebrate===2?"Becoming a Yellow Jacket":"Academic Takeoff"}!</h2><p><b>Way to go, {studentName}!</b> You completed every required mission for this stage and arrived at the final checkpoint.</p><div className="level-awards"><span>{String(celebrate).padStart(2,"0")} <small>LEVEL {celebrate} BADGE</small></span><span>👕 <small>{celebrate===2?"GT T-SHIRT":"GT HOODIE"}</small></span><span>🏠 <small>DORM COLLECTION</small></span></div><div className="reward-room-preview"><DormRoom items={roomItems} level={celebrate} preview={previewReward}/></div>{!claimedReward?<><h3>🎁 Choose one reward for your dorm</h3><div className="reward-options">{(roomRewards[celebrate]||roomRewards[2]).map(item=><button key={item} onMouseEnter={()=>setPreviewReward(item)} onFocus={()=>setPreviewReward(item)} onClick={()=>chooseReward(item)} className={previewReward===item?"selected":""}><span>{rewardEmoji[item]}</span><b>{item}</b><small>Preview & choose</small></button>)}</div><p className="reward-note">Preview each item, then choose one to place it immediately.</p></>:<motion.div className="reward-claimed" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><span>{rewardEmoji[claimedReward]}</span><div><small>ADDED TO YOUR DORM</small><b>{claimedReward}</b></div><button onClick={continueToNextLevel}>{celebrate===2?"Begin Academic Takeoff":"Begin Welcome Home"} <ArrowRight/></button></motion.div>}</motion.div></motion.div>}
      {finalReady&&<motion.div className="final-ready" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>{Array.from({length:12}).map((_,i)=><motion.span key={i} initial={{y:"110vh",x:0}} animate={{y:"-20vh",x:(i%2?1:-1)*(30+i*4)}} transition={{duration:3+(i%4),repeat:Infinity,delay:i*.18}}>🐝</motion.span>)}<motion.div initial={{scale:.8,y:25}} animate={{scale:1,y:0}}><small>FIRST FLIGHT COMPLETE</small><h2>You’re officially ready, {studentName}!</h2><p>You completed your essential onboarding, registered for your first semester, connected with your roommate, and prepared your new home.</p><div><b>🎓 Classes planned</b><b>🏠 Dorm ready</b><b>🐝 Yellow Jacket ready</b></div><h3>Welcome to your freshman year at Georgia Tech.</h3><button onClick={()=>{setFinalReady(false);setShowRoom(true)}}>See my finished dorm <ArrowRight/></button></motion.div></motion.div>}
