...
export default function HomePage() {
  ...
  const [roomItems] = useState<string[]>(() => savedState()?.roomItems || []);
  ...
  useEffect(() => { localStorage.setItem("first-flight-state", JSON.stringify({version:2,currentLevel,emailCursor,tasks,sideMissions,avatarColor,accessory,onboarded,guideDismissed,avatarType,skin,hair,roomItems,merchItems,studentName,major})); }, [currentLevel,emailCursor,tasks,sideMissions,avatarColor,accessory,onboarded,guideDismissed,skin,hair,roomItems,merchItems,studentName,major]);
  useEffect(() => {
  ...
  function continueToNextLevel() { const next=level+1;if(next===2){setCurrentLevel(2);setTasks(level2Tasks);setEmailCursor(5)}else if(next===3){setCurrentLevel(3);setTasks(level3Tasks);setEmailCursor(8)}else{setFinalReady(true)}setCelebrate(null);setClaimedReward("");setPreviewReward("");setShowRoom(false);if(next<=3){setToast(`Level ${next} · ${next===2?"Academic Takeoff":"Welcome Home"} unlocked!`);setTimeout(()=>setToast(""),3000);setTimeout(()=>setEmailPopup(sampleEmails[next===2?5:8]),1600)}}
  ...
  return <main className="app-shell">
    ...
    <AnimatePresence>
      ...
      {onboarded && !guideDismissed && <motion.div className="progress-guide" initial={{opacity:0}} animate={{opacity:1}}><motion.div initial={{scale:.92,y:22}} animate={{scale:1,y:0}}><div className="guide-badge">01</div><small>YOUR BUZZ MAP JOURNEY</small><h2>Welcome aboard, {studentName}!</h2><p>Your journey to becoming a Yellow Jacket is divided into levels. Each level contains important onboarding missions to help you arrive prepared.</p><div className="guide-loop"><div><span>✓</span><b>Complete missions</b><small>Finish essential GT onboarding tasks</small></div><ArrowRight/><div><span>XP</span><b>Earn experience</b><small>Watch your journey come to life</small></div><ArrowRight/><div><span>🎟</span><b>Unlock coupons</b><small>Choose free simulated GT merch</small></div></div><div className="guide-rewards"><div><span>👕</span><p><b>Level 1 merch</b>Choose a GT T-shirt, Buzz cap, or water bottle.</p></div><div><span>🎒</span><p><b>Better gear each level</b>Complete later levels to unlock premium merch choices.</p></div></div><button className="complete-button" onClick={()=>setGuideDismissed(true)}>Begin Level 1 <ArrowRight/></button></motion.div></motion.div>}
      ...
      {celebrate && <motion.div className="celebration reward-celebration" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>{Array.from({length:24}).map((_,i)=><motion.i key={i} style={{left:`${10+(i*37)%80}%`,background:["#edbd66","#fff0b9","#65c1b4","#d887aa"][i%4]}} initial={{y:-80,rotate:0}} animate={{y:"105vh",rotate:720}} transition={{duration:2+(i%5)*.31,delay:(i%7)*.07}}/>)}<motion.div className="reward-card merch-reward-card" initial={{scale:.5,y:30}} animate={{scale:1,y:0}} transition={{type:"spring",delay:.15}}><Sparkles/><small>🎉 LEVEL {celebrate-1} COMPLETE!</small><h2>You unlocked a free GT merch coupon!</h2><p><b>Way to go, {studentName}!</b> Pick one item below. This is a simulated hackathon reward coupon—not a real Georgia Tech offer.</p><div className="coupon-ticket"><span>BUZZ MAP REWARD</span><b>ONE FREE MERCH ITEM</b><em>LEVEL {celebrate-1} · DEMO COUPON</em></div>{!claimedReward?<><h3>🎟️ Choose your free merch</h3><div className="reward-options merch-options">{(merchCoupons[celebrate]||merchCoupons[2]).map(item=><button key={item} onMouseEnter={()=>setPreviewReward(item)} onFocus={()=>setPreviewReward(item)} onClick={()=>chooseReward(item)} className={previewReward===item?"selected":""}><span>{merchEmoji[item]}</span><b>{item}</b><small>Select this coupon</small></button>)}</div><p className="reward-note">Your choice will be saved in your Buzz Map reward collection.</p></>:<motion.div className="reward-claimed" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}><span>{merchEmoji[claimedReward]}</span><div><small>COUPON CLAIMED</small><b>Free {claimedReward}</b></div><button onClick={continueToNextLevel}>{celebrate===2?"Begin Academic Takeoff":celebrate===3?"Begin Welcome Home":"Finish my journey"} <ArrowRight/></button></motion.div>}</motion.div></motion.div>}
